...
public class PenaltyTest {
    ...
    public void setup() {
        ...
        when(Logger.getLogger(Penalty.class.getName())).thenReturn(logger);
    ...
    public void executeSaveByDefense() {
        ...
        PowerMockito.when(gameResolver.getPenaltyChance()).thenReturn(-1.0);
        PowerMockito.when(gameResolver.getPenaltyToGoalChance()).thenReturn(-1.0);
        ...
        IPenalty penalty = factory.createPenalty(gameplayConfig);
        penalty.execute(teamInGameOne, teamInGameTwo);
        assertEquals(1, playerInGame.getSaves());
    }

    @Test
    public void executeSaveByGoalie() {
        PowerMockito.when(gameplayConfig.getGameResolver()).thenReturn(gameResolver);
        PowerMockito.when(gameResolver.getPenaltyChance()).thenReturn(2.0);
        PowerMockito.when(gameResolver.getPenaltyToGoalChance()).thenReturn(-1.0);
        PowerMockito.when(teamInGameTwo.getGoalieOnIce()).thenReturn(defense);
        playerInGame = factory.createPlayerInGame(player);
        PowerMockito.when(defense.getRandomPlayer()).thenReturn(playerInGame);
    ...
    public void executeGoal() {
        PowerMockito.when(gameplayConfig.getGameResolver()).thenReturn(gameResolver);
        PowerMockito.when(gameResolver.getPenaltyChance()).thenReturn(2.0);
        PowerMockito.when(gameResolver.getPenaltyToGoalChance()).thenReturn(2.0);
